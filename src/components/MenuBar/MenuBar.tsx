...
export default function MenuBar() {  
  ...
  useEffect(() => {
    ...
    if (String(URLRoomName).length>0 && String(URLidentity).length>0) {
      handleSubmits();
    }
  }, [URLRoomName,URLidentity]);

  const handleSubmits = () => {
    if (!window.location.origin.includes('twil.io')) {
      window.history.replaceState(null, '', window.encodeURI(`/room/${roomName}${window.location.search || ''}`));
    }
    getToken(String(URLidentity),String(URLRoomName)).then(token => connect(token));
  };
